...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'adventurers' 
    AND policyname = 'Users can view their own adventurers'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'adventurers' 
    AND policyname = 'Users can insert their own adventurers'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'adventurers' 
    AND policyname = 'Users can update their own adventurers'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'adventurer_stat_history' 
    AND policyname = 'Users can view their own stat history'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'adventurer_stat_history' 
    AND policyname = 'Users can insert their own stat history'
  ) THEN
...
DROP TRIGGER IF EXISTS update_adventurer_timestamp ON adventurers;
